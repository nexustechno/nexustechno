            ...
            <form action="{{route('addAgentBanking')}}" method="post" name="frmbanking" id="frmbanking">
            	...
            	<table class="table custom-table balance-table white-bg text-color-blue-2 search-result dataTable no-footer" id="pager1">
                    ...
                    ?>
                        ...
                        ?>
                   			@endforeach
                             <tr><td><b>Total</b></td><td><b>{{number_format($totBal,2, '.', '')}}</b></td><td ><b>{{number_format($totAvl,2, '.', '')}}</b></td><td><span class="text-color-red"><b>({{number_format(abs($totExp),2, '.', '')}})</b></span></td><td></td><td><b style="justify-content: center;display: flex;">{{number_format($totcRef,2, '.', '')}}</b></td><td class="{{$classref}}"><b>({{number_format(abs($totrPL),2, '.', '')}})</td><td></td></tr>
                   	</tbody>
            	</table>
