...
function search($array, $key, $value)
{
	...
	public function getExAmount_Session($fancyName,$eventid,$uid,$deductamt,$position,$betside)
	{
		...
		if(sizeof($new_obj)>0)
		{
			...
			$bet_chk=''; $bet_model=''; $final_exposer=0; 
	...
	public function MyBetStore(Request $request)
	{
		...
		if($headerUserBalance <= 0){			
			$responce['status']='false';
			$responce['msg']='Insufficent Balance!111';
			return json_encode($responce);
			exit;
		}
		...
		{
			$responce['status']='false';
			$responce['msg']='Insufficent Balance!222';
			return json_encode($responce);
			exit;
		}
		$isExBalEq = false;
		...
		if($requestData['bet_type'] === 'ODDS')
		{
			...
			if($headerUserBalance < ($exposureAmt+$deduct_expo_amt+$exposureAmt_session))
			{
				if($headerUserBalance < ($exposureAmt+$deduct_expo_amt+$exposureAmt_session) && $exposureAmt<=0)
				{  	
					$responce['status']='false';
					$responce['msg']='Insufficent Balance!333';
					return json_encode($responce);
					exit;
				}
				$fval = $stack;
				...
				if($requestData['bet_type'] === 'ODDS')
				{
					...
					if($requestData['bet_side'] == 'lay')
					{
					   ...
					   {
							$responce['status']='false';
							$responce['msg']='Insufficent Balance!4444';
							return json_encode($responce);
							exit;
					   }
					   /*echo 'fancy total--'.$fancy_exposer; echo "<br>";
					  ...
					  //echo  $headerUserBalance .'<'. $other_bet_placed_amount.'+'.abs($total_negative_number).'+'.$other_bet_session;
					  // exit;
					   if($headerUserBalance < ($other_bet_placed_amount+abs($total_negative_number)+$other_bet_session)) //put this condition 27-8-2021
					   {
						   	$responce['status']='false';
							$responce['msg']='Insufficent Balance!5555';
							return json_encode($responce);
							exit;
					   }
				...
				{
					$responce['status']='false';
					$responce['msg']='Insufficent Balance!666';
					return json_encode($responce);
					exit;
				}
				...
				{
					$responce['status']='false';
					$responce['msg']='Insufficent Balance!777';
					return json_encode($responce);
					exit;
				}
			...
			if($requestData['bet_type'] === 'SESSION')
			{
				...
				{
					$responce['status']='false';
					$responce['msg']='Insufficent Balance!888';
					return json_encode($responce);
					exit;
				}
				...
				{
					$responce['status']='false';
					$responce['msg']='Insufficent Balance!999';
					return json_encode($responce);
					exit;
				}
			}
			...
			if(isset($exAmtArr[$requestData['bet_type']][$requestData['team_name']][$requestData['bet_type']."_profitLost"]))
			{
				...
				if($requestData['bet_side'] == 'lay')
				{
					...
					else
					{
						...
						{
							$responce['status']='false';
							$responce['msg']='Insufficent Balance!10 10 10';
							return json_encode($responce);
							exit;
						}
				  	}
				  	if(isset($requestData['teamname1']) && isset($exAmtArr[$requestData['bet_type']][$requestData['teamname1']][$requestData['bet_type']."_profitLost"]))
					{
					  	...
					  	if($betprofit1 >= 0)
						{
							...
							{
								$responce['status']='false';
								$responce['msg']='Insufficent Balance!11 11 11';
								return json_encode($responce);
								exit;
						  	}
					  	}
				  	...
				  	if(isset($requestData['teamname2']) && !empty($requestData['teamname2']) && isset($exAmtArr[$requestData['bet_type']][$requestData['teamname2']][$requestData['bet_type']."_profitLost"]))
					{
						...
						if($betprofit2 >= 0)
						{
							...
							if($amt < 0)
							{
								...
								{
									$responce['status']='false';
									$responce['msg']='Insufficent Balance!12 12 12';
							  		return json_encode($responce);
							  		exit;
								}
					  		}
					...
					if(isset($requestData['teamname3']) && !empty($requestData['teamname3']) && isset($exAmtArr[$requestData['bet_type']][$requestData['teamname3']][$requestData['bet_type']."_profitLost"]))
					{
					  	...
					  	if($betprofit3 >= 0)
						{
							...
							{
								$responce['status']='false';
								$responce['msg']='Insufficent Balance!13 13 13';
								return json_encode($responce);
								exit;
								}
							}
		...
		if($requestData['bet_type'] == 'SESSION')
		{
			...
			if($headerUserBalance < ($exposureAmt+$exposureAmt_session+$exposureAmt_SESSION))
			{
				$responce['status']='false';
				$responce['msg']='Insufficent Balance!14 14 14';
				return json_encode($responce);
				exit;
			}
		...
		{
			$responce['status']='false';
			$responce['msg']='Insufficent Balance!15 15 15';
			return json_encode($responce);
			exit;	 
		}
	 	else
  ...
  public function frontAutoLogout(Request $request)
  {
    ...
    $cricket_count = count($this->search($arrayA, 'inplay', '1'));    
    ...
    $arrayt = json_decode($return, true);
    $tennis_count = count($this->search($arrayt, 'inplay', '1')); 
    ...
    $arrayc = json_decode($return, true);
    $soccer_count = count($this->search($arrayc, 'inplay', '1'));     
