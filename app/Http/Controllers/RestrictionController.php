...
class RestrictionController extends Controller
{
    ...
    public function agentSubDetail(Request $request)
    {
        ...
        $user = User::where('parentid', $user_id)->orderBy('user_name')->get();
        $admin = Auth::user()->id;
        ...
        // end calculation    
            ...
            if($row->agent_level == 'PL'){
                ...
                <tr>
                    ...
                    $html2.='<td class="white-bg text-color-red" style="display:table-cell;">('.number_format(abs($exposure_cli),2, '.', '').')</td>';
        ...
        foreach ($user as $key => $row) {
            ...
            if($row->agent_level == 'PL'){
                ...
                <tr>
                    ...
                    $html2.='<td class="white-bg '.$refPL.'">('.number_format(abs($refPL),2, '.', '').')</td>
                    <td class="'.$cuClass.' white-bg">('.number_format(abs($cumulative_pl_cli),2, '.', '').')</td>
                    <td class="white-bg" style="display: table-cell;"> 
                    ';
               ...
               $cuClass='text-color-red';
                ...
                $html.='
                    <tr>
                        ...
                        <td class="white-bg">'.number_format($calData[1],2, '.', '').'</td>
                        <td class="white-bg">'.number_format($total_calculated_available_balance,2, '.', '').'</td>
                        <td class="white-bg text-color-red">('.number_format(abs($calData[2]),2, '.', '').')</td>
                        <td class="white-bg '.$class.'" style="display:table-cell;">('.number_format(abs($refPL),2, '.', '').')</td>
                        <td class="'.$cuClass.' white-bg">('.number_format(abs($calData[3]),2, '.', '').')</td>
                        <td class="white-bg" style="display: table-cell;"> 
                        ';
