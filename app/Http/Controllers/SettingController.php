...
class SettingController extends Controller
{
	...
	public function getFancyBetPositionForMatchDeclare($fancyName,$mid,$eventid,$uid,$fancy_result)
	{
		...
		$return_final_exposure=''; $profit_loss=""; $final_exposer_deduct='';
		if(sizeof($my_placed_bets)>0)
		{
			$run_arr=array();
			foreach($my_placed_bets as $bet)
			...
			$bet_chk=''; $bet_model='';
			$final_exposer=''; $expo_array=array();
	...
	public function getFancyBetResult($fancyname,$matchid,$eventid,$id,$result)
	{
		...
		if(sizeof($my_placed_bets)>0)
		{
            ...
            Log::info('total_expo_amount: ',$total_expo_amount);
			if($mytotal > 0){
                $total_expo_amount = $mytotal-(int)$check_value[2];
                Log::info('total_expo_amount: ',$total_expo_amount);
            }
	...
	public function risk_management_details($id)
	{
      ...
      if(!empty($getUserparent5->parentid)){
		...
		foreach ($users_all as $value)
		{
			...
			if(sizeof($my_placed_bets_ods)>0)
		    {
			...
			$adminBookUser .= '
              <tfoot class="ng-scope" style="background-color: #fff;" data-ng-repeat="MDLlist in MDLDataList track by MDLlist.mdlUsername">
                <tr class="trMDl">
                  <td style="width: 215px;" class="text-left cursor-pointer" data-toggle="collapse" data-target="#SPname-'.$i.'"><b class="ng-binding">'.$value->user_name.'['.$value->first_name.']</b>
                  </td>
                  <td class="text-center ng-binding '.$clsa1.'" >'.abs($team1_bet_total_user).'</td>
                  <td class="text-center ng-bindingsdsd '.$clsa2.'" >'.abs($team2_bet_total_user).'</td>
                  <td class="text-center ng-binding '.$clsa2.'" >'.abs($team_draw_bet_total_user).'</td>
                  <!-- ngIf: MDLlist.runner3Name!=null -->
                </tr>';

            $adminBookUser.='<tr id="SPname-'.$i.'" class="collapse">
                  <td colspan="4">
                    <table style="background: #d5f6f7;width: 100%" class="table-hover">
                      <!-- ngRepeat: DLlist in MDLlist.dlBook track by DLlist.dlUsername -->
                      <tbody data-ng-repeat="DLlist in MDLlist.dlBook track by DLlist.dlUsername" class="ng-scope">';
                    $users_allSP = User::where('parentid', $value->id)->latest()->get();
                    $users_allSPcount = User::where('parentid', $value->id)->latest()->count();
                    if($users_allSPcount != 0){
                    foreach ($users_allSP as $valuesp)
					{
			$team2_bet_total_sp=0;
			$team1_bet_total_sp=0;
			$team_draw_bet_total_sp=0;
			$my_placed_bets_odssp=array();
			if($valuesp->agent_level == 'PL')
			{
			 $my_placed_bets_odssp = MyBets::where('match_id',$matchList->event_id)->where('bet_type','ODDS')->where('result_declare',0)->where('isDeleted',0)->where('user_id', $valuesp->id)->orderby('id','DESC')->get();

			}else{
				$all_childs = UserHirarchy::where('agent_user',$valuesp->id)->first();
				$all_childsCount = UserHirarchy::where('agent_user',$valuesp->id)->count();
				 if($all_childsCount!=0){

                  $all_child = $this->GetChildofAgent($valuesp->id);
                 $my_placed_bets_odssp = MyBets::where('match_id',$matchList->event_id)->where('bet_type','ODDS')->where('result_declare',0)->where('isDeleted',0)->whereIn('user_id', $all_child)->orderby('id','DESC')->get();
			}
			}
			if(sizeof($my_placed_bets_odssp)>0)
		    {
		    	 foreach($my_placed_bets_odssp as $bet)
		        {
		            ...
		            if(count($abc)>=2)
		            {
		                if (array_key_exists("teamname1",$abc) && array_key_exists("teamname2",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team1_bet_total_sp=$team1_bet_total_sp+$bet->exposureAmt;
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_sp=$team_draw_bet_total_sp-$bet->bet_profit;
		                        }
		                        $team2_bet_total_sp=$team2_bet_total_sp+$bet->exposureAmt;
		                    }
		                    if($bet->bet_side=='lay')
		                    {
		                        $team1_bet_total_sp=$team1_bet_total_sp-($bet->bet_amount);
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_sp=$team_draw_bet_total_sp+($bet->exposureAmt);
		                        }
		                        $team2_bet_total_sp=$team2_bet_total_sp-($bet->bet_amount);
		                    }
		                }
		                else if (array_key_exists("teamname3",$abc) && array_key_exists("teamname2",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team1_bet_total_sp=$team1_bet_total_sp-$bet->bet_profit;
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_sp=$team_draw_bet_total_sp+$bet->exposureAmt;
		                        }
		                    ...
		                    if($bet->bet_side=='lay')
		                    {
		                        ...
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_sp=$team_draw_bet_total_sp-($bet->bet_amount);
		                        }
		                        $team2_bet_total_sp=$team2_bet_total_sp-($bet->bet_amount);
		                    }
		                }
		                else if (array_key_exists("teamname3",$abc) && array_key_exists("teamname1",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team2_bet_total_sp=$team2_bet_total_sp-($bet->bet_profit);
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_sp=$team_draw_bet_total_sp+$bet->exposureAmt;
		                        }
            ...
            $adminBookUser.='<tr>
                          <td style="width:209px;" class="text-left cursor-pointer" data-toggle="collapse" data-target="#SPPname-0-'.$i.'"><b class="ng-binding">'.$valuesp->user_name.'['.$valuesp->first_name.']</b></td>
                          <td class="text-center ng-binding redclass">'.$team1_bet_total_sp.'</td>
                          <td class="text-center ng-binding greenclass">'.$team2_bet_total_sp.'</td>
                          <!-- ngIf: DLlist.runner3Name!=null -->
                        </tr>';
            $users_allth = User::where('parentid', $valuesp->id)->latest()->get();
            foreach ($users_allth as $valueth)
					{
			$team2_bet_total_th=0;
			$team1_bet_total_th=0;
			$team_draw_bet_total_th=0;
			$my_placed_bets_odsth=array();
			if($valueth->agent_level == 'PL')
			{
			 $my_placed_bets_odsth = MyBets::where('match_id',$matchList->event_id)->where('bet_type','ODDS')->where('result_declare',0)->where('isDeleted',0)->where('user_id', $valueth->id)->orderby('id','DESC')->get();

			}else{
				$all_childs = UserHirarchy::where('agent_user',$valueth->id)->first();
				$all_childsCount = UserHirarchy::where('agent_user',$valueth->id)->count();
				 if($all_childsCount!=0){

                  $all_child = $this->GetChildofAgent($valueth->id);
                 $my_placed_bets_odsth = MyBets::where('match_id',$matchList->event_id)->where('bet_type','ODDS')->where('result_declare',0)->where('isDeleted',0)->whereIn('user_id', $all_child)->orderby('id','DESC')->get();

			}
			}
			if(sizeof($my_placed_bets_odsth)>0)
		    {
		    	 foreach($my_placed_bets_odsth as $bet)
		        {
		            ...
		            if(count($abc)>=2)
		            {
		                if (array_key_exists("teamname1",$abc) && array_key_exists("teamname2",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team1_bet_total_th=$team1_bet_total_th+$bet->exposureAmt;
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_th=$team_draw_bet_total_th-$bet->bet_profit;
		                        }
		                        $team2_bet_total_th=$team2_bet_total_th+$bet->exposureAmt;
		                    }
		                    if($bet->bet_side=='lay')
		                    {
		                        $team1_bet_total_th=$team1_bet_total_th-($bet->bet_amount);
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_th=$team_draw_bet_total_th+($bet->exposureAmt);
		                        }
		                        $team2_bet_total_th=$team2_bet_total_th-($bet->bet_amount);
		                    }
		                }
		                else if (array_key_exists("teamname3",$abc) && array_key_exists("teamname2",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team1_bet_total_th=$team1_bet_total_th-$bet->bet_profit;
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_th=$team_draw_bet_total_th+$bet->exposureAmt;
		                        }
		                    ...
		                    if($bet->bet_side=='lay')
		                    {
		                        ...
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_th=$team_draw_bet_total_th-($bet->bet_amount);
		                        }
		                        $team2_bet_total_th=$team2_bet_total_th-($bet->bet_amount);
		                    }
		                }
		                else if (array_key_exists("teamname3",$abc) && array_key_exists("teamname1",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team2_bet_total_th=$team2_bet_total_th-($bet->bet_profit);
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_th=$team_draw_bet_total_th+$bet->exposureAmt;
		                        }
            ...
            $adminBookUser.='<tr id="SPPname-0-'.$i.'" class="collapse">
                          <td colspan="4" data-toggle="collapse" data-target="#thirdname-0-'.$i.'"><b class="ng-binding">
                            <table style="background: honeydew;width: 100%" class="table-hover">
                              <!-- ngRepeat: CLlist in DLlist.clientBook track by CLlist.userName -->
                              <tbody data-ng-repeat="CLlist in DLlist.clientBook track by CLlist.userName" class="ng-scope">
                                <tr>
                                  <td style="width:200px;" class="text-left"><b class="ng-binding">'.$valueth->user_name.'['.$valueth->first_name.']</b></td>
                                  <td class="text-center ng-binding redclass">'.$team1_bet_total_th.'</td>
                                  <td class="text-center ng-binding greenclass">'.$team2_bet_total_th.'</td>
                                  <!-- ngIf: CLlist.runner3Name!=null -->
                                </tr>
                              </tbody>
                        ...
                        ';
            $users_allf = User::where('parentid', $valueth->id)->latest()->get();
            foreach ($users_allf as $valuef)
			{
		    $team2_bet_total_f=0;
			$team1_bet_total_f=0;
			$team_draw_bet_total_f=0;
			if($valuef->agent_level == 'PL')
			{
			 $my_placed_bets_odsf = MyBets::where('match_id',$matchList->event_id)->where('bet_type','ODDS')->where('result_declare',0)->where('isDeleted',0)->where('user_id', $valuef->id)->orderby('id','DESC')->get();

			}else{
				$all_childs = UserHirarchy::where('agent_user',$valuef->id)->first();
				$all_childsCount = UserHirarchy::where('agent_user',$valuef->id)->count();
				 if($all_childsCount!=0){

                 $all_child = $this->GetChildofAgent($valuef->id);
                 $my_placed_bets_odsf = MyBets::where('match_id',$matchList->event_id)->where('bet_type','ODDS')->where('result_declare',0)->where('isDeleted',0)->whereIn('user_id', $all_child)->orderby('id','DESC')->get();

			}
			}
			if(sizeof($my_placed_bets_odsf)>0)
		    {
		    	foreach($my_placed_bets_odsf as $bet)
		        {
		            ...
		            if(count($abc)>=2)
		            {
		                if (array_key_exists("teamname1",$abc) && array_key_exists("teamname2",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team1_bet_total_f=$team1_bet_total_f+$bet->exposureAmt;
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_sp=$team_draw_bet_total_sp-$bet->bet_profit;
		                        }
		                        $team2_bet_total_f=$team2_bet_total_f+$bet->exposureAmt;
		                    }
		                    if($bet->bet_side=='lay')
		                    {
		                        $team1_bet_total_f=$team1_bet_total_f-($bet->bet_amount);
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_sp=$team_draw_bet_total_sp+($bet->exposureAmt);
		                        }
		                        $team2_bet_total_f=$team2_bet_total_f-($bet->bet_amount);
		                    }
		                }
		                else if (array_key_exists("teamname3",$abc) && array_key_exists("teamname2",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team1_bet_total_f=$team1_bet_total_f-$bet->bet_profit;
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_sp=$team_draw_bet_total_sp+$bet->exposureAmt;
		                        }
		                ...
		                else if (array_key_exists("teamname3",$abc) && array_key_exists("teamname1",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team2_bet_total_f=$team2_bet_total_f-($bet->bet_profit);
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_sp=$team_draw_bet_total_sp+$bet->exposureAmt;
		                        }
            ...
            $adminBookUser.='<tr id="thirdname-0-'.$i.'" class="collapse">
                          <td colspan="4" data-toggle="collapse" data-target="#fourname-0-'.$i.'"><b class="ng-binding">
                            <table style="background: honeydew;width: 100%" class="table-hover">
                              <!-- ngRepeat: CLlist in DLlist.clientBook track by CLlist.userName -->
                              <tbody data-ng-repeat="CLlist in DLlist.clientBook track by CLlist.userName" class="ng-scope">
                                <tr>
                                  <td style="width:200px;" class="text-left"><b class="ng-binding">'.$valuef->user_name.'['.$valuef->first_name.']</b></td>
                                  <td class="text-center ng-binding redclass">'.$team1_bet_total_f.'</td>
                                  <td class="text-center ng-binding greenclass">'.$team2_bet_total_f.'</td>
                                  <!-- ngIf: CLlist.runner3Name!=null -->
                                </tr>
                              </tbody>
                        ...
                        ';
            $users_allfi = User::where('parentid', $valuef->id)->latest()->get();
            foreach ($users_allfi as $valuefi)
			{
			$team2_bet_total_fi=0;
			$team1_bet_total_fi=0;
			$team_draw_bet_total_fi=0;
			if($valuefi->agent_level == 'PL')
			{
			 $my_placed_bets_odsfi = MyBets::where('match_id',$matchList->event_id)->where('bet_type','ODDS')->where('result_declare',0)->where('isDeleted',0)->where('user_id', $valuefi->id)->orderby('id','DESC')->get();

			}else{
				$all_childs = UserHirarchy::where('agent_user',$valuefi->id)->first();
				$all_childsCount = UserHirarchy::where('agent_user',$valuefi->id)->count();
				 if($all_childsCount!=0){

                  $all_child = $this->GetChildofAgent($valuefi->id);
                 $my_placed_bets_odsfi = MyBets::where('match_id',$matchList->event_id)->where('bet_type','ODDS')->where('result_declare',0)->where('isDeleted',0)->whereIn('user_id', $all_child)->orderby('id','DESC')->get();

			}
			}
			if(sizeof($my_placed_bets_odsfi)>0)
		    {
		    	foreach($my_placed_bets_odsfi as $bet)
		        {
		            ...
		            if(count($abc)>=2)
		            {
		                if (array_key_exists("teamname1",$abc) && array_key_exists("teamname2",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team1_bet_total_fi=$team1_bet_total_fi+$bet->exposureAmt;
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_fi=$team_draw_bet_total_fi-$bet->bet_profit;
		                        }
		                        $team2_bet_total_fi=$team2_bet_total_fi+$bet->exposureAmt;
		                    }
		                    if($bet->bet_side=='lay')
		                    {
		                        $team1_bet_total_fi=$team1_bet_total_fi-($bet->bet_amount);
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_fi=$team_draw_bet_total_fi+($bet->exposureAmt);
		                        }
		                        $team2_bet_total_fi=$team2_bet_total_fi-($bet->bet_amount);
		                    }
		                }
		                else if (array_key_exists("teamname3",$abc) && array_key_exists("teamname2",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team1_bet_total_fi=$team1_bet_total_fi-$bet->bet_profit;
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_fi=$team_draw_bet_total_fi+$bet->exposureAmt;
		                        }
		                    ...
		                    if($bet->bet_side=='lay')
		                    {
		                        ...
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_fi=$team_draw_bet_total_fi-($bet->bet_amount);
		                        }
		                        $team2_bet_total_fi=$team2_bet_total_fi-($bet->bet_amount);
		                    }
		                }
		                else if (array_key_exists("teamname3",$abc) && array_key_exists("teamname1",$abc))
		                {
		                    ...
		                    if($bet->bet_side=='back')
		                    {
		                        $team2_bet_total_fi=$team2_bet_total_fi-($bet->bet_profit);
		                        if(count($abc)>=2)
		                        {
		                            $team_draw_bet_total_fi=$team_draw_bet_total_fi+$bet->exposureAmt;
		                        }
            ...
            $adminBookUser.='<tr id="fourname-0-'.$i.'" class="collapse">
                      <td colspan="4" data-toggle="collapse" data-target="#thirdnamess-0-'.$i.'"><b class="ng-binding">
                        <table style="background: honeydew;width: 100%" class="table-hover">
                          <!-- ngRepeat: CLlist in DLlist.clientBook track by CLlist.userName -->
                          <tbody data-ng-repeat="CLlist in DLlist.clientBook track by CLlist.userName" class="ng-scope">
                            <tr>
                              <td style="width:200px;" class="text-left"><b class="ng-binding">'.$valuefi->user_name.'['.$valuefi->first_name.']</b></td>
                              <td class="text-center ng-binding redclass">'.$team1_bet_total_fi.'</td>
                              <td class="text-center ng-binding greenclass">'.$team2_bet_total_fi.'</td>
                              <!-- ngIf: CLlist.runner3Name!=null -->
                            </tr>
                          </tbody>
		...
		foreach ($users_all as $value)
		{
			...
			$adminBookUserBM .= '
              <tfoot class="ng-scope" style="background-color: #fff;" data-ng-repeat="MDLlist in MDLDataList track by MDLlist.mdlUsername">
                <tr class="trMDl">
                  <td style="width: 215px;" class="text-left cursor-pointer" data-toggle="collapse" data-target="#SPnameB-'.$i.'"><b class="ng-binding">'.$value->user_name.'['.$value->first_name.']</b>
                  </td>
                  <td class="text-center ng-binding" style="color: red;">'.$team1_bet_total_user.'</td>
                  <td class="text-center ng-binding" style="color: green;">'.$team2_bet_total_user.'</td>
                  <!-- ngIf: MDLlist.runner3Name!=null -->
                </tr>';

            $adminBookUserBM.='<tr id="SPnameB-'.$i.'" class="collapse">
                  <td colspan="4">
                    <table style="background: #d5f6f7;width: 100%" class="table-hover">
                      <!-- ngRepeat: DLlist in MDLlist.dlBook track by DLlist.dlUsername -->
                      <tbody data-ng-repeat="DLlist in MDLlist.dlBook track by DLlist.dlUsername" class="ng-scope">';
			...
			if($valuesp->agent_level == 'PL')
			{
			 $my_placed_bets_odssp = MyBets::where('match_id',$matchList->event_id)->where('bet_type','BOOKMAKER')->where('result_declare',0)->where('isDeleted',0)->where('user_id', $valuesp->id)->orderby('id','DESC')->get();
				...
				$all_childsCount = UserHirarchy::where('agent_user',$valuesp->id)->count();
				 if($all_childsCount!=0){
                  ...
                  $all_child = $this->GetChildofAgent($valuesp->id);
                 $my_placed_bets_odssp = MyBets::where('match_id',$matchList->event_id)->where('bet_type','BOOKMAKER')->where('result_declare',0)->where('isDeleted',0)->whereIn('user_id', $all_child)->orderby('id','DESC')->get();
			}
			}
			if(sizeof($my_placed_bets_odssp)>0)
		    {
            ...
            $adminBookUserBM.='<tr>
                          <td style="width:209px;" class="text-left cursor-pointer" data-toggle="collapse" data-target="#SPPnameB-0-'.$i.'"><b class="ng-binding">'.$valuesp->user_name.'['.$valuesp->first_name.']</b></td>
                          <td class="text-center ng-binding redclass">'.$team1_bet_total_sp.'</td>
                          <td class="text-center ng-binding greenclass">'.$team2_bet_total_sp.'</td>
                          <!-- ngIf: DLlist.runner3Name!=null -->
                        </tr>';
			...
			if($valueth->agent_level == 'PL')
			{
			 $my_placed_bets_odsth = MyBets::where('match_id',$matchList->event_id)->where('bet_type','BOOKMAKER')->where('result_declare',0)->where('isDeleted',0)->where('user_id', $valueth->id)->orderby('id','DESC')->get();
				...
				$all_childsCount = UserHirarchy::where('agent_user',$valueth->id)->count();
				 if($all_childsCount!=0){
                  ...
                  $all_child = $this->GetChildofAgent($valueth->id);
                 $my_placed_bets_odsth = MyBets::where('match_id',$matchList->event_id)->where('bet_type','BOOKMAKER')->where('result_declare',0)->where('isDeleted',0)->whereIn('user_id', $all_child)->orderby('id','DESC')->get();
            ...
            $adminBookUserBM.='<tr id="SPPnameB-0-'.$i.'" class="collapse">
                          <td colspan="4" data-toggle="collapse" data-target="#thirdnameB-0-'.$i.'"><b class="ng-binding">
                            <table style="background: honeydew;width: 100%" class="table-hover">
                              <!-- ngRepeat: CLlist in DLlist.clientBook track by CLlist.userName -->
                              <tbody data-ng-repeat="CLlist in DLlist.clientBook track by CLlist.userName" class="ng-scope">
                                <tr>
			...
			if($valuef->agent_level == 'PL')
			{
			 $my_placed_bets_odsf = MyBets::where('match_id',$matchList->event_id)->where('bet_type','BOOKMAKER')->where('result_declare',0)->where('isDeleted',0)->where('user_id', $valuef->id)->orderby('id','DESC')->get();
				...
				$all_childsCount = UserHirarchy::where('agent_user',$valuef->id)->count();
                 ...
                 $all_child = $this->GetChildofAgent($valuef->id);
                 $my_placed_bets_odsf = MyBets::where('match_id',$matchList->event_id)->where('bet_type','BOOKMAKER')->where('result_declare',0)->where('isDeleted',0)->whereIn('user_id', $all_child)->orderby('id','DESC')->get();
            ...
            $adminBookUserBM.='<tr id="thirdnameB-0-'.$i.'" class="collapse">
                          <td colspan="4" data-toggle="collapse" data-target="#fournameB-0-'.$i.'"><b class="ng-binding">
                            <table style="background: honeydew;width: 100%" class="table-hover">
                              <!-- ngRepeat: CLlist in DLlist.clientBook track by CLlist.userName -->
                              <tbody data-ng-repeat="CLlist in DLlist.clientBook track by CLlist.userName" class="ng-scope">
                                <tr>
			...
			if($valuefi->agent_level == 'PL')
			{
			 $my_placed_bets_odsfi = MyBets::where('match_id',$matchList->event_id)->where('bet_type','BOOKMAKER')->where('result_declare',0)->where('isDeleted',0)->where('user_id', $valuefi->id)->orderby('id','DESC')->get();
				...
				$all_childsCount = UserHirarchy::where('agent_user',$valuefi->id)->count();
				 if($all_childsCount!=0){
                  ...
                  $all_child = $this->GetChildofAgent($valuefi->id);
                 $my_placed_bets_odsfi = MyBets::where('match_id',$matchList->event_id)->where('bet_type','BOOKMAKER')->where('result_declare',0)->where('isDeleted',0)->whereIn('user_id', $all_child)->orderby('id','DESC')->get();
            ...
            $adminBookUserBM.='<tr id="fournameB-0-'.$i.'" class="collapse">
                      <td colspan="4" data-toggle="collapse" data-target="#thirdnamess-0-'.$i.'"><b class="ng-binding">
                        <table style="background: honeydew;width: 100%" class="table-hover">
                          <!-- ngRepeat: CLlist in DLlist.clientBook track by CLlist.userName -->
                          <tbody data-ng-repeat="CLlist in DLlist.clientBook track by CLlist.userName" class="ng-scope">
            ...
            $adminBookUserBM.='</tfoot>';
        	$i++;
    		}
		}

      	return view('backpanel/risk-management-details',compact('matchList','my_placed_bets','html','my_placed_bets_BM','html_BM','html_Fancy','managetv','list','my_placed_bets_fancy','users_all','team1_bet_total','team2_bet_total','team_draw_bet_total','adminBookUser','adminBookUserBM','team2_bet_totalB','team1_bet_totalB','team_draw_bet_totalB'));
	}
	...
	public function rollback_user_bet(Request $request)
	{
		...
		if($del)
		{
			...
			//echo $getc->exposure.'-'.abs($original_expo).'+'.abs($new_expo);  echo "<br>";
			$updc->exposure = $getc->exposure-abs($original_expo)+abs($new_expo);
			$updc->available_balance_for_D_W = $getc->available_balance_for_D_W+abs($original_expo)-abs($new_expo);
			$upd=$updc->update();
			if($upd)
	...
	public function risk_management_details_ajax($id,Request $request)
	{
		...
		if($match_data!=0)
		{
			...
			if($match_m=='0')
			{
				...
				{
				/*echo "aas".$match_data[0]['runners'][2]['ex']['availableToBack'][0]['price'];
				exit;*/
					if(@$match_data[0]['runners'][2]['ex']['availableToBack'][0]['price']!='')
					{
						$display=''; $cls='';
						...
						<td></td>
						<td class="fancy-suspend-td" colspan="6">
							<div class="fancy-suspend black-bg-5 text-color-white" style="height:41px;"><span>SUSPENDED</span></div>
						</td>
					</tr><tr class="rf_tr">
							<td class="text-left"> <img src="'.asset('asset/front/img/bars.png').'"> <b class="team3"> THE DRAW </b> <br>
								<div>
					...
					else
					{
						...
						<tr class="fancy-suspend-tr team3_fancy">
							<td></td>
							<td class="fancy-suspend-td" colspan="6">
								<div class="fancy-suspend black-bg-5 text-color-white" style="height: 42px !important;"><span>SUSPENDED</span></div>
							</td>
						</tr>
						<tr class="rf_tr white-bg">
						<td> <img src="'.asset('asset/front/img/bars.png').'"> <b class="team1">  '.strtoupper($team[0]).' </b>
	...
	public function addPlayerBanking(Request $request)
	{
		...
		if (Hash::check($apass, $adm_password))
	    {
			...
			else
			{
				...
				foreach($player as $play)
				{
					$btype=''; $amount=0; $rem_balance=0; $credit_amount=0; $credit_balance=0;  $remark=''; $available_balance=0; $av_balance=0;
					...
					if($request->txtamount[$i]!='' && $request->txtamount[$i]>0)
					{
						...
						$play_client->available_balance_for_D_W = $play_client->remain_bal - $play_client->exposure;
						$play_client->update();
					}
	...
	public function getFancyBetPosition($fancyName,$mid,$eventid,$uid,$fancy_result)
	{
		...
		$abc=sizeof($my_placed_bets);
		$return_final_exposure=''; $profit_loss=""; $return_exposure=0; $expo_array=array();
		if(sizeof($my_placed_bets)>0)
		{
			$run_arr=array();
			foreach($my_placed_bets as $bet)
			...
			$bet_chk=''; $bet_model=''; $final_exposer='';
